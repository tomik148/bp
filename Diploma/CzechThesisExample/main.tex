...
<Label Content="{Shared:Localization Key='PŘEKLADOVÝ.KLÍČ', Default='Defaultní text'}"/>
		...
		\subsubsection{Akce}
			Jsou 3 druhy akcí uložené procedury, EService a Assembly. Uložené procedury se dají přidávat a editovat přímo z této aplikace, jsou podobné filtrům ale místo aby vracely tabulku tak něco změní. EService jsou microservisy založené nad technologií RabbitMQ. A pokud zákazník potřebuje nějakou funkci (např. uložit soubor na disk) která nelze uspokojit předchozíma dvěma druhy požije se Assembly.
