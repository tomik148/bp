...
\Thanks{Rád bych na tomto místě poděkoval všem mým kolegům v práci, mému vedoucímu práce, mému konzultantovi ve firmě a všem ostatním, kteří mi s prací pomohli.}
...
\CooperatingPersonsDeclarationImageFile{Figures/CoopPersonDeclaration.png}
...
\section{Moje úkoly}
	...
	\subsection{Použité technologie}
		...
		\subsubsection{WPF}
			...
			WPF podporuje návrhový vzor MVVM\cite{mvvm}, který stejně jako další MVX vzory odděluje data od prezentační vrstvy, čehož je docíleno pomocí bindingu. Což je mechanizmus, který synchronizuje data, která jsou ve View a ViewModel. To znamená, že pokud uživatel zadá něco do text boxu, tak se to propíše i do property, která je na daný TextBox navázána. Pokud ViewModel implementuje rozhraní \uv{INotifyPropertyChanged}, bude tento proces fungovat i opačným směrem.
			...
			View jsou napsána v značkovacím jazyce XAML\cite{xaml}, jenž je založený na XML. Kde jednotlivé tagy jsou reprezentace tříd v {\Csharp} (např. \uv{<sys:Int32>} kde \uv{sys} je namespace \uv{System}, je třída \uv{Int32} ). XAML umožňuje editaci za běhu programu, což bylo velmi užitečné při stylování. 
	...
	\subsection{\EFilters}
		{\EFilters} je desktopová aplikace, kterou jsem měl za úkol vytvořit. Tato část aplikace {\EvolioEight} (v ní pod názvem \uv{Filtry} vice v podkapitole \ref{txt:filters}) by jako webová aplikace fungovala hůře.
...
<Label Content="{Shared:Localization Key='PŘEKLADOVÝ.KLÍČ', Default='Defaultní text'}"/>
			\end{lstlisting}
			 ...
			 Kde \uv{Shared} je namespace ve kterém se nachází daná třída, \uv{Localization} je MarkupExtension třída, \uv{Key} je property na třídě \uv{Localization}, která obsahuje překladový klíč a property \uv{Default} obsahuje výchozí text, který se zobrazí pokud překlad není dostupný.
		\subsubsection{Filtry}
		\label{txt:filters}
			Filter je rozdělen do několika tabulek, tabulka \uv{Filters} obsahuje jméno, SQL dotaz a jméno procedury spolu s dalšími nepovinnými daty. 
			
			Dále tabulka \uv{FilterProperties} obsahuje data pro jednotlivé property filtru údaje jako jsou jméno, datový typ, přesnost, velikost, zda je parametr povinný a také data pro CDS. 
...
\section{Využité a scházející znalosti a dovednosti}
	\subsection{Využité znalosti}
		Při odborné praxi jsem využil několik znalostí získaných při studiu. Práci s {\Csharp} a ASP.NET pomohly předměty \uv{Architektura technologie .NET} a \uv{Programovací jazyky II}. S databází mi pomohly předměty \uv{Úvod do databázových systémů} a \uv{Databázové a informační systémy}. A nejednou mi pomohla moje znalost verzovacího systému GIT.
